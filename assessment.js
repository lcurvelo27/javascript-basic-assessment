...
var removeItem = (array, string) => {
	var copy = array.slice()
	for(var i = 0; i < array.length; i++){
		if(copy.indexOf(string) != -1){
			copy.splice(copy.indexOf(string), 1)
		}
	}
	return copy
}
...
var getValueOfProperty = (obj, prop) => {
	return obj[prop]
}
...
var makeChatMessage = (msg, author) => {
	return {
		message: msg,
		author: author,
		timestamp: new Date()
...
//to have a property called lovesCode with a value of 10.
...
// #15 Create a function called outside that takes in a temperature (number),
//a humidity(number), and a cloudiness(number), in that order. Using the following to return the correct values
/*
...
var outside = (temp, humidity, cloudiness) => {
